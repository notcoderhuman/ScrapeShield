...
for (const [status, expectedScore, expectedUsable] of [['healthy', '100/100', '5/5'], ['degraded', '80/100', '4/5'], ['failed', '0/100', '0/5']]) test(`rendering ${status} demo leaves loading state`, async () => { const selectors = await executeApp(demoResponse(status)); assert.equal(selectors.get('#health-heading').textContent, status.toUpperCase()); assert.equal(selectors.get('#collector-id').textContent, 'c_test'); assert.match(selectors.get('#checked-at').textContent, /Last checked/); assert.equal(selectors.get('#health-score').textContent, expectedScore); assert.equal(selectors.get('#hero-usable').textContent, expectedUsable); assert.equal(selectors.get('#hero-risk').textContent, status === 'degraded' ? '1' : '0'); assert.equal(selectors.get('#field-list').children.length, 5); });
