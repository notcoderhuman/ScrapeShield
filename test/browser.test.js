...
test('dashboard consumes current reliability API fields', () => { for (const field of ['latestRun', 'runHistorySummary', 'fieldDiagnostics', 'drift', 'runHistory', 'recoveryEvent', 'incidents', 'fieldReliability']) assert.match(app, new RegExp(field)); });
test('field reliability panel has derived evidence and safe demo empty state', () => { assert.match(html, /Field reliability/); assert.match(html, /Derived from observed live field evidence/); assert.match(app, /Insufficient live evidence/); assert.match(app, /MOST AFFECTED/); });
test('dashboard has concise incident titles, complete evidence, and safe empty state', () => { assert.match(html, /Incidents/); assert.match(app, /Collector output incident/); assert.match(app, /Affected: \$\{incident\.affectedFields\.map\(title\)/); assert.match(app, /Observed by ScrapeShield/); assert.match(app, /External repair: not observed by ScrapeShield/); assert.match(app, /No simulated incidents/); });
...
test('dashboard renders isolated demo history and timeline states', () => { for (const phrase of ['Fields at risk', 'What changed', 'Run history', 'Recovery history', 'Health trend']) assert.match(html, new RegExp(phrase, 'i')); for (const phrase of ['No structured output changes detected', 'Run history will appear', 'Field history will appear', 'No simulated recovery events', 'SIMULATED FIXTURE TIMELINE', 'Demo events are isolated from live collector history']) assert.match(app, new RegExp(phrase)); assert.match(app, /if \(data\.demo\).*No simulated recovery events/); assert.match(app, /if \(data\.demo\).*SIMULATED FIXTURE TIMELINE/); });
